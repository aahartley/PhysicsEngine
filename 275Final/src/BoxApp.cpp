...
void Application::setup() {
	...
	running = Graphics::openWindow("Austin Hartley Assignment5");	//open a window for rendering
	...
	for (int i = 0; i < 3; i++) {
		Body* bigBox = new Body(Box(4, 4), x, y, -1);
		bigBox->rotation = -3.14159 / 4;
		bigBox->restitution = 0.7;
		bigBox->color = 0xFF404040;
		y -= 4 * std::sqrt(2) + 0.344;
		bodies.push_back(bigBox);
	}
	x = -4;
	y = std::sqrt(2) * 0.5 * 4 + 1;
	for (int i = 0; i < 2; i++) {
		Body* bigBox = new Body(Box(4, 4), x, y, -1);
		bigBox->rotation = -3.14159 / 4;
		bigBox->restitution = 0.7;
...
void Application::input() {
	...
	while (SDL_PollEvent(&event)) {
		...
		case SDL_MOUSEBUTTONDOWN:
			...
			Body* box = new Body(Box(2, 2), Graphics::xPosInCoordinate(x),
				Graphics::yPosInCoordinate(y), 1.0);
