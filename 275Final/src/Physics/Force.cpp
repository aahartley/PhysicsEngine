...
Vec2 Force::generateDragForce(const Particle& particle, float k) {
	...
	Vec2 dragDirection = particle.velocity.unitVector()*-1.0;
        ...
        float dragMagnitude = k * particle.velocity.magnitude(); //sqaured to not
...
Vec2 Force::generateWindForce(const Particle& particle, float k, float wind) {
	Vec2 windForce = Vec2(wind, 0);


	// Generate the final wind force with direction and magnitude
	windForce *=k;
	//windForce -= particle.velocity;
	//}
	return windForce;
}
