...
void Application::setup() {
	running = Graphics::openWindow("Austin Hartley Assignment4");//openWindow();
	...
	Body* cueDisk = new Body(Disk(1.5), -15, 0, 10);
	cueDisk->color = 0xFFFFFFFF;
	bodies.push_back(cueDisk);

	Body* targetDisk = new Body(Disk(1.5), 15, 0, 10);
	targetDisk->color = 0xFF00FF00;
	bodies.push_back(targetDisk);

	Body* targetDisk2 = new Body(Disk(1.5), 17.7, 1.5, 10);
	targetDisk2->color = 0xFF0055FF;
	bodies.push_back(targetDisk2);

	Body* targetDisk3 = new Body(Disk(1.5), 17.7, -1.5, 10);
	targetDisk3->color = 0xFFFFFF00;
	bodies.push_back(targetDisk3);
}
...
void Application::input() {
	...
	while (SDL_PollEvent(&event)) {
		...
		case SDL_MOUSEBUTTONUP:
			if (leftMouseButtonDown && event.button.button == SDL_BUTTON_LEFT) {
				leftMouseButtonDown = false;
				bodies[0]->velocity = (bodies[0]->position - Graphics::windowToCoordinate(mouseCursor)) * 4.5;	// simply set up the new velocity
			}
			break;
		}
	}
...
void Application::update() {
	...
	for (auto body : bodies) {
		...
		Vec2 friction = Force::generateFrictionForce(*body, 200);
		if (body->velocity.magnitude() > .01)
